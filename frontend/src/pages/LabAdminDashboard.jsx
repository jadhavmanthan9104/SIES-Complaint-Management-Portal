...
const LabAdminDashboard = () => {
  ...
  useEffect(() => {
    const data = localStorage.getItem("lab_admin_data");
    if (data) {
  ...
  const fetchComplaints = async () => {
    try {
      ...
      const response = await axios.get(`${BACKEND_URL}/api/lab-complaints`, {
  ...
  const handleStatusChange = async (complaintId, newStatus) => {
    try {
      const token = localStorage.getItem("lab_admin_token");
      await axios.patch(
        `${BACKEND_URL}/api/lab-complaints/${complaintId}/status`,
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("lab_admin_data");
    toast.success("Logged out successfully");
    navigate("/lab/admin/auth");
  };
        ...
        <div className="bg-white/70 dark:bg-slate-900/60 backdrop-blur-xl rounded-2xl shadow-xl border border-white/40 dark:border-slate-700/50 p-4 md:px-6 flex flex-col md:flex-row items-center justify-between gap-4 transition-all duration-300">
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div className="flex items-center gap-2">
              <div className="bg-blue-100 p-2 rounded-full dark:bg-slate-800">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 relative z-10">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="bg-white/40 dark:bg-slate-800/40 px-4 py-2 rounded-xl border border-white/30 dark:border-slate-700/30 text-slate-800 dark:text-slate-200 font-medium">
            Total: <span data-testid="complaint-count" className="text-blue-600 dark:text-blue-400 font-bold ml-1">{complaints.length}</span>
          </div>
        </div>
          ...
          <div className="grid gap-6">
            {complaints.map((complaint) => (
              <Card key={complaint.id} className="p-6 bg-white/70 dark:bg-slate-900/60 backdrop-blur-md border border-white/50 dark:border-slate-700/50 shadow-lg hover:shadow-xl transition-all duration-300" data-testid="complaint-card">
                <div className="grid md:grid-cols-2 gap-6 mb-6">
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Lab Number</p>
                        <p className="font-bold text-blue-600 dark:text-blue-400" data-testid="complaint-lab">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Contact Info</p>
                      <div className="text-sm text-slate-700 dark:text-slate-300 space-y-0.5">
                        <p data-testid="complaint-email">{complaint.email}</p>
                        <p>{complaint.phone}</p>
                      </div>
                    </div>
                  </div>
                ...
                {complaint.photo_base64 && (
                  <div className="mb-6 bg-slate-50 dark:bg-slate-800/30 p-4 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                    <p className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Attached Evidence</p>
                    <img
                      src={complaint.photo_base64}
                      alt="Complaint Evidence"
                      className="w-full max-w-sm h-48 object-cover rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 cursor-pointer hover:opacity-90 transition-opacity"
                    />
                  </div>
                )}
